...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:bg-zinc-950 dark:border-zinc-800">
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-1 text-sm font-medium">
            <Link href="/shop" className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition">Shop</Link>
            {/* SECURITY ▼ */}
            <div className="relative group">
              <button className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition flex items-center gap-1 font-medium">Security <span className="text-xs opacity-60 group-hover:rotate-180 transition-transform">▾</span></button>
              <div className="absolute left-0 top-full hidden group-hover:block group-focus-within:block bg-white border-2 border-[#7FAF25]/10 rounded-xl shadow-xl w-64 mt-2 z-50 overflow-hidden">
                <div className="p-2 space-y-1">
                  {SERVICES.filter(s=>s.cat==="Security").map(s=><Link key={s.slug} href={s.href} className="block px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#3F5D13] text-sm">{s.title}</Link>)}
                </div>
              </div>
            </div>
            {/* IT & NETWORKING ▼ */}
            <div className="relative group">
              <button className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition flex items-center gap-1 font-medium">IT & Networking <span className="text-xs opacity-60 group-hover:rotate-180 transition-transform">▾</span></button>
              <div className="absolute left-0 top-full hidden group-hover:block group-focus-within:block bg-white border-2 border-[#7FAF25]/10 rounded-xl shadow-xl w-72 mt-2 z-50 overflow-hidden">
                <div className="p-2 space-y-1">
                  {SERVICES.filter(s=>s.cat==="IT & Networking").map(s=><Link key={s.slug} href={s.href} className="block px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#3F5D13] text-sm">{s.title}</Link>)}
                </div>
              </div>
            </div>
            {/* POWER & SOLAR ▼ */}
            <div className="relative group">
              <button className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition flex items-center gap-1 font-medium">Power & Solar <span className="text-xs opacity-60 group-hover:rotate-180 transition-transform">▾</span></button>
              <div className="absolute left-0 top-full hidden group-hover:block group-focus-within:block bg-white border-2 border-[#7FAF25]/10 rounded-xl shadow-xl w-72 mt-2 z-50 overflow-hidden">
                <div className="p-2 space-y-1">
                  {SERVICES.filter(s=>s.cat==="Power & Solar").map(s=><Link key={s.slug} href={s.href} className="block px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#3F5D13] text-sm">{s.title}</Link>)}
                  <div className="border-t my-1" />
                  <Link href="/services/maintenance" className="block px-3 py-2 rounded-lg hover:bg-[#F2F9E6] text-sm">Maintenance & Repair</Link>
                  <Link href="/estates" className="block px-3 py-2 rounded-lg hover:bg-[#F2F9E6] text-sm font-medium text-[#7FAF25]">Estate Solutions</Link>
                </div>
              </div>
            </div>
            <Link href="/estates" className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition">Estates</Link>
            <Link href="/about" className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition">About</Link>
            <Link href="/blog" className="px-3 py-2 rounded-lg hover:bg-[#F2F9E6] hover:text-[#7FAF25] transition">Blog</Link>
          </nav>

          <div className="flex items-center gap-1">
            <Link href="/shop" className="relative">
              <Button variant="ghost" size="icon" className="hover:text-[#7FAF25] relative">
                <Heart className="h-5 w-5" />
                {wishlist.length > 0 && <span className="absolute -top-1 -right-1 bg-[#7FAF25] text-black text-[10px] font-bold rounded-full h-4 w-4 grid place-items-center">{wishlist.length}</span>}
              </Button>
            </Link>
            <Button variant="ghost" size="icon" className="relative hover:text-[#7FAF25]" onClick={() => setCartOpen(true)}>
              <ShoppingCart className="h-5 w-5" />
              {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-[#7FAF25] text-black text-[10px] font-bold rounded-full h-4 w-4 grid place-items-center">{cartCount}</span>}
            </Button>
            {session?.user ? (
              <>
                {(session.user as any).role === "ADMIN" ? (
                  <Link href="/admin" className="hidden sm:inline-flex"><Button variant="secondary" size="sm" className="bg-[#0A0A0A] text-white hover:bg-black"><User className="h-4 w-4" /> Dashboard</Button></Link>
                ) : (
      ...
      <div className="border-t bg-[#F9FBEF]/70">
        <div className="container mx-auto px-4 py-2.5">
          <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-zinc-500" />
            <Input placeholder="Search CCTV, solar, biometrics, gates... (39 products)" className="pl-10 pr-20 bg-white border-[#7FAF25]/20 focus:border-[#7FAF25] h-10 shadow-sm" value={query} onChange={(e) => setQuery(e.target.value)} />
      ...
      {mobileOpen && (
        <div className="container mx-auto px-4 lg:hidden border-t py-4 space-y-3">
          ...
          <div className="pl-2 border-l-2 border-[#7FAF25]/20 ml-1 space-y-1">
            <p className="text-xs font-bold uppercase tracking-widest text-[#7FAF25]">Services</p>
          ...
          {session?.user ? (
            <>
              {(session.user as any).role === "ADMIN" && <Link href="/admin" className="block py-2 font-semibold text-[#7FAF25]">Dashboard</Link>}
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-[60] flex justify-end">
          ...
          <div className="relative bg-white w-full max-w-md h-[100vh] shadow-2xl flex flex-col">
            <div className="p-4 border-b flex items-center justify-between bg-[#0A0A0A] text-white">
              <h3 className="font-black flex items-center gap-2"><ShoppingCart className="h-5 w-5 text-[#7FAF25]" /> Cart ({cartCount})</h3>
                ...
                cart.map((item) => (
                  <div key={`${item.productId}-${item.slug}`} className="flex gap-3 border rounded-xl p-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-bold text-sm text-[#0A0A0A]">{formatKES(item.price)} × {item.qty} = {formatKES(item.price * item.qty)}</p>
                    </div>
            ...
            {cart.length > 0 && (
              <div className="border-t p-4 space-y-3 bg-zinc-50">
                <div className="flex justify-between font-black text-lg"><span>Total</span><span className="text-[#0A0A0A]">{formatKES(cartTotal)}</span></div>
