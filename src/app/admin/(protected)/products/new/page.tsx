      ...
      <Card className="border-2 border-[#7FAF25]/20">
        ...
        <CardContent>
          <form action="/api/products" method="post" className="grid md:grid-cols-2 gap-3">
            <input name="name" placeholder="Product name *" required className="md:col-span-2 border-2 border-zinc-200 focus:border-[#7FAF25] rounded-lg px-3 py-2.5 text-sm outline-none" />
            <input name="slug" placeholder="slug e.g. hikvision-ptz-4mp" required className="border-2 border-zinc-200 focus:border-[#7FAF25] rounded-lg px-3 py-2.5 text-sm outline-none" />
            <select name="category" required className="border-2 border-zinc-200 focus:border-[#7FAF25] rounded-lg px-3 py-2.5 text-sm outline-none bg-white">
              <option value="">Category *</option>
              {categories.map(c=> <option key={c} value={c}>{c.replace("_"," ")}</option>)}
            </select>
            <input name="price" type="number" placeholder="Price KES *" required className="border-2 border-zinc-200 focus:border-[#7FAF25] rounded-lg px-3 py-2.5 text-sm outline-none" />
            ...
            <input name="image" placeholder="Image URL (Unsplash) *" required className="md:col-span-2 border-2 border-zinc-200 focus:border-[#7FAF25] rounded-lg px-3 py-2.5 text-sm outline-none" defaultValue="https://images.unsplash.com/photo-1558002038-1055907df827?auto=format&fit=crop&w=800&q=80" />
            ...
            <div className="md:col-span-2 flex gap-3">
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="active" defaultChecked className="accent-[#7FAF25]" /> Active</label>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="installationAvailable" className="accent-[#7FAF25]" /> Installation available</label>
            </div>
            <Button type="submit" className="md:col-span-2 h-11">Create Product</Button>
            <p className="md:col-span-2 text-xs text-zinc-500 text-center">After create, you’ll see it in <Link href="/shop" className="text-[#5A7F1B] underline">/shop</Link> and dashboard. Use Prisma Studio for advanced edits: <code>npm run db:studio</code></p>
          </form>
        </CardContent>
      </Card>
    </div>
