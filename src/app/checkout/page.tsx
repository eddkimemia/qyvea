...
export default function CheckoutPage() {
  ...
  if (typeof window !== "undefined" && new URLSearchParams(window.location.search).get("success")) {
    return (
      <div className="container mx-auto px-4 py-12 max-w-2xl text-center">
        <div className="h-16 w-16 rounded-full bg-[#7FAF25] text-black grid place-items-center mx-auto text-2xl">✓</div>
  ...
  return (
    <div className="container mx-auto px-4 py-6 md:py-8">
      <Link href="/shop" className="inline-flex items-center gap-1 text-sm text-zinc-600 hover:text-[#7FAF25] mb-4"><ArrowLeft className="h-4 w-4" /> Back to Shop</Link>
      ...
      {cart.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-6 mt-6">
          {/* Form */}
          <Card className="border-2 border-[#7FAF25]/10">
            ...
            <CardContent>
              <form onSubmit={handlePlaceOrder} className="space-y-4">
                ...
                <label className="flex items-center gap-2 text-sm font-medium p-3 border-2 border-[#7FAF25]/20 rounded-xl bg-[#F2F9E6] cursor-pointer">
                  <input type="checkbox" checked={form.includeInstall} onChange={(e) => setForm({ ...form, includeInstall: e.target.checked })} className="accent-[#7FAF25] h-4 w-4" /> Include professional installation (+ KES 3,500 est.)
                </label>
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1.5 text-sm pt-2">
                  ...
                  <div className="flex justify-between"><span className="text-zinc-500">Delivery</span><span className={deliveryFee === 0 ? "text-[#7FAF25] font-bold" : ""}>{deliveryFee === 0 ? "FREE" : formatKES(deliveryFee)}</span></div>
                ...
                <div className="grid grid-cols-3 gap-2 text-[11px] text-center pt-2">
                  <div className="border rounded-lg p-2"><Truck className="h-4 w-4 mx-auto text-[#7FAF25]" /><p className="font-semibold">Free Nairobi</p></div>
                  <div className="border rounded-lg p-2"><ShieldCheck className="h-4 w-4 mx-auto text-[#7FAF25]" /><p className="font-semibold">5-Yr Warranty</p></div>
            ...
            <Card className="bg-[#0A0A0A] text-white">
              <CardContent className="p-4 text-sm">
                <p className="font-bold text-[#7FAF25]">Need help checkout?</p>
